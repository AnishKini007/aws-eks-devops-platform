# VPC Module - Creates a production-ready VPC with multi-AZ subnets
...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${var.name}-nat-${count.index + 1}-${var.environment}"
    }
  )
...
resource "aws_route_table" "database" {
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${var.name}-database-rt-${var.environment}"
    }
  )
}
